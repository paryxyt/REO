function [F, gradF, H, fconstraint] = barrier_REC_LP(Y,varargin)
% Inputs: c, K_params, dims are the objective function, cone parameters and dims
% respectively of the problem
c=varargin{1};
K_params=varargin{2};
dims=varargin{3}; %[N; m; m1; n1; n2; ns] where ns is number of slack vars
...
%%%%%%%%% Orthant part
fconstraint_orth=Y2;
F_orth=-sum(log(Y2)); %self-concordant barrier for z>=0
grad_orth=-(1./Y2);
H_orth=diag(1./(Y2.^2));
...
F = F_exp + F_orth;
gradF = [grad_exp; grad_orth] + c;
H = [H_exp zeros(n1,n2); zeros(n2,n1) H_orth];
